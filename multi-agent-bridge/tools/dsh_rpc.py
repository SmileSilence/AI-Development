...
任务适配器通过工作区 ID 原子创建会话，DSH 桌面窗口无需重启即可看到新会话。
...
def verify_session_membership(workspace_id, session_id):
    """以自锚点无操作排序读取工作区投影，同时验证会话已归属。"""
...
def rename_workspace(workspace_id, title):
    return unary("workspace/rename", {
        "request": {"workspaceId": workspace_id, "title": title}
    })

def rename_session(session_id, title):
    return unary("session/rename", {
        "request": {"sessionId": session_id, "title": title}
    })

def archive_session(session_id):
    return unary("workspace/archiveSession", {
        "request": {"sessionId": session_id}
    })

def delete_workspace(workspace_id):
    return unary("workspace/delete", {
