---
name: code-style
description: "提供通用代码风格、命名、注释与审查规范；编写、格式化、重构或审查代码时使用。"
metadata:
  publisher: SmileXX
  version: "v8"
  short-description: 代码规范与代码风格指南
  category: 代码规范
---
...
- v8 (2026-08-29): 精简技能目录描述，保留通用编码与审查触发条件
- v7 (2026-08-28): 调整适用范围：本技能为通用规范，普遍适用于所有编码/生成/重构流程；引擎/框架特有规范优先，冲突以外仍须遵循本技能通用规范
- v6 (2026-08-28): 新增防御编程规范（外部输入校验、标准路径 API、平台判断与 fallback）；由 SmileProjectConfig 移入
- v5 (2026-08-27): 优化触发条件：细化自动触发与不触发场景，技能自包含（不引用其他技能）
- v4 (2026-09-02): 错误处理小节新增异步数据一致性规范（异步调用数据须就绪后使用，避免同步调用消费异步数据）
