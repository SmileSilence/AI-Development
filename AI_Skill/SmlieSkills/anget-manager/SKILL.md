---
name: anget-manager
description: 管理 AI Agent 启动、技能清单安装、会话同步与临时文件；涉及 anget、DSH、Codex、Claude 或 Gemini 时使用。
metadata:
  publisher: SmileXX
  version: "v8"
  short-description: 管理 Agent 启动、会话版本合并同步与临时文件
  category: Agent 管理
---
...
10. **版本链收敛**：Claude 或 dsh 同一源会话更新后复用稳定暂存路径；Codex 原生导入复用当前任务 ID 时直接更新该任务，只有产生不同任务 ID 时才归档旧版本。
11. **活跃会话防抖**：最后修改不足 60 秒的 Claude/dsh 源会话视为仍在写入，本轮跳过；稳定满 60 秒后再同步，避免连续生成不完整版本。
12. **同步保护**：不修改源会话；自动排除子代理和空会话；导入后核验并清理暂存目录。
13. **技能清单安装**：读取 `SKILL_CATALOG.xlsx` 的“总技能清单”工作表，按“默认安装”字段自动安装推荐技能并跳过非推荐技能；执行自有技能安装脚本时，由 `skill-manifest.json` 提供对应的机器可读字段。
...
8. 只有全部替代会话导入成功，才调用 `thread/archive` 归档对应旧任务及同一 Claude/dsh 来源的旧版本；必须排除本批导入返回的目标任务 ID，归档可恢复。
...
用户提供 `SKILL_CATALOG.xlsx` 时：

1. 读取 Excel 的“总技能清单”工作表，以“默认安装”列作为安装范围依据。
2. “默认安装=是”的技能直接安装；“否”的技能默认跳过。
3. SmileXX 自有技能只克隆一次 AI-Development 仓库，然后运行 `pwsh -File .\AI_Skill\install-skills.ps1 -RecommendedOnly`。
   安装脚本读取 `AI_Skill/skill-manifest.json` 中的 `default_install` 字段，不要求额外的总清单 JSON。
...
| 日期 | 版本 | 变更说明 |
|------|------|----------|
| 2026-08-29 | v8 | 删除重复总清单 JSON 依赖；总览读取 Excel，自有技能安装由 skill-manifest.json 驱动 |
| 2026-08-29 | v7 | 新增总技能清单自动安装规则，支持按默认安装字段筛选与验证 |
| 2026-08-29 | v6 | 精简技能目录描述，保留 Agent 管理与会话同步触发词 |
| 2026-08-28 | v5 | 新增同源版本链识别、原任务原地更新保护及活跃会话 60 秒防抖 |
