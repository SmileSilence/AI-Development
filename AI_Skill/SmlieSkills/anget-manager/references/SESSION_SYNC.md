...
5. 规范会话编号由“工作区＋来源标识”稳定生成，正文更新后仍复用同一暂存路径，重复检测不会产生随机副本。
6. 同组任一来源已按当前内容导入时，复用原有 Codex 会话，不再生成新的规范副本。
7. dsh 会兼容识别 v2 版本使用的 `DSH-Import` 导入登记，防止技能升级后重复导入。

## 同源版本链规则

1. Claude 使用原 JSONL 路径作为来源标识；dsh 使用稳定会话编号对应的旧 `DSH-Import` 路径作为来源标识。
   整理后的暂存文件名由工作区和来源标识稳定生成，不随正文更新而变化。
2. 同一来源内容变化并成功导入新整理结果后，读取该来源此前的 `imported_thread_id`。
3. Codex 原生导入若复用已有目标任务 ID，则直接更新该任务且不得归档它；只有返回不同目标任务 ID 时，才在整批成功后归档其他未归档旧版本。
4. 导入失败、结果数量不一致或归档失败时必须报告，禁止删除旧版本。
5. 版本收敛与正文指纹去重同时执行：同内容跨来源合并，同来源新旧内容按版本链收敛。
6. 最后修改不足 60 秒的源文件视为活跃会话，本轮只报告为 `active` 并跳过；稳定满 60 秒后才进入比较和导入。
...
7. 归档只用于成功生成替代会话的反向来源任务或同源旧版本，归档失败必须单独报告且不得删除任务。
