...
脚本会自动检查 Python `zstandard` 依赖、调用 `sync-agent-sessions.py`，输出工作区、输入会话、反向差异、合并数量和候选会话统计。最后修改不足 60 秒的活跃源会话会暂时跳过。正式导入全部成功后会归档已被合并替代的旧 Codex 任务及同源旧版本；若原生导入复用当前目标任务 ID，则直接更新并排除归档。成功或失败后均清理任务临时目录。详细规则见 `SESSION_SYNC.md`。
