...
- **并行评估**：解析需求后，评估是否存在相互独立、不阻塞主体流程且不存在修改冲突的子任务。
- **子代理分派**：平台支持子代理且任务复杂度足以抵消协作成本时，将适合并行处理的子任务分派给子代理，主代理同时推进主体任务。
- **冲突控制**：不得将存在强依赖关系或可能同时修改相同文件的任务并行分派。
- **结果整合**：主代理负责检查、整合并验证子代理结果，对最终交付质量负责。
