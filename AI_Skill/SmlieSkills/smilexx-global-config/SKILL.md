...
- **报告归档**：修改后的报告文档（评审、分析、报告等）统一放入项目 `<项目目录>\docs\reports\`（目录规范见 smilexx-project-config）
...
**技能经验回写（自我更新）：**
- **触发时机**：使用任何技能完成任务的过程中，出现下列任一情况时，须在当次会话内及时回写，不得拖延或遗忘：
  1. 中间处理过的问题（经额外排查、绕行或验证才解决）；
  2. 被用户纠正的错误；
  3. 自查发现的错误（步骤缺失、命令失效、描述过时或有歧义）。
- **回写内容**：更新对应技能的 SKILL.md（必要时含 references/ 与 scripts/）：写明原来的错误之处、正确的方法与步骤、经验总结，使下次使用可直接避坑。
- **回写方式**：先完整阅读目标技能再局部修改；同步更新其 metadata.version 与 last-updated 并在版本记录中登记；完成后运行 `pwsh -File .\AI_Skill\install-skills.ps1` 同步到安装目录。
- **归属判断**：单技能的经验写入该技能；跨技能的通用规则写入本技能（smilexx-global-config）。
...
- **适用阶段**：在设计阶段和 Plan 模式中执行本规则。
- **先查后问**：先根据现有上下文、相关技能、文档和只读检查查明可发现事实，不向用户询问可自行确认的问题。
- **关键疑问必问**：若仍存在无法自行查明且会影响方案的疑问，必须向用户提问，不得擅自替用户作出关键决策。
- **说明提问原因**：提问时必须同时说明为什么需要确认，以及用户的答案会影响哪些方案决策。
- **减少无效打断**：不为无关或不影响方案的问题打断用户；非关键细节采用合理默认值，并在计划中明确记录假设。

推荐表述：

> 需要确认 X，因为它会决定 Y 的实现方式；如果选择 A，则……，如果选择 B，则……。
...
| 2026-09-03 | v17.1 | 补充 metadata.platforms 与 metadata.keywords 字段，符合 smilexx-skill-creator v7 元数据规范 |
...
| 2026-08-28 | v13 | 技能名改为 kebab-case（smilexx-global-config），符合 DSH 加载规范 |
