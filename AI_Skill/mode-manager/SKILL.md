---
name: mode-manager
description: 管理 AI Agent 工作模式：读取并显示当前模式（mode-state.json），在 6 种模式（架构、编码、调试、多模式协调、分析、提问）间切换并持久化。支持 Codex、Claude、DeepSeek Harness、MiMo Code。当用户询问「当前模式」「现在是什么模式」或要求「切换到X模式」「切到编码/调试/分析」「进入架构模式」「/mode」时使用。
metadata:
  version: "v2"
---
...
### 自动触发
- 用户询问「当前模式」「现在是什么模式」
- 用户要求「切换到 X 模式」「切到编码/调试/分析」「进入架构模式」
- 消息以 /mode 开头（如 `/mode coding`）
- 用户说「mode-manager」「模式管理」

### 不触发（避免误用）
- 单纯的编码/调试/分析任务本身（未要求切换模式）→ 不切换，按当前模式执行
- 用户提到「架构」「调试」等词但只是讨论概念，无切换意图

### 判定要点
- 以「是否要求查看或切换工作模式」为准；不含切换意图时仅按当前模式执行
...
| 日期 | 版本 | 变更说明 |
|------|------|----------|
| 2026-08-27 | v2 | 优化触发条件：补充不触发场景与判定要点，避免概念性讨论误触发 |
| 2026-08-22 | v1 | 初始版本 |
