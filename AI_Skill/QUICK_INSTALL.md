...
| 技能 | 版本 | 作用 |
|------|------|------|
| smilexx-coding-workflow | v1.3 | AI 辅助编码工作流与通用代码规范 |
| smilexx-global-config | v21 | 全局中文配置与 Agent 管理 |
| smilexx-unity-development | v3.2 | Unity 项目开发指南 |
| smilexx-unity-plugin-development | v3.2 | Unity 插件 / Package 开发指南 |
...
### 方式二：只安装默认推荐的 4 个技能（手动复制）

```powershell
$skills = @("smilexx-coding-workflow","smilexx-global-config","smilexx-unity-development","smilexx-unity-plugin-development")
...
```powershell
# 只安装 smilexx-unity-development
Copy-Item -Path ".\AI_Skill\SmlieSkills\smilexx-unity-development" -Destination "$env:USERPROFILE\.agents\skills\" -Recurse -Force
```

DSH 插件开发可按需复制 `SmlieSkills/smilexx-dsh-plugin-creator` 到共享目录，并另行复制到 `~/.claude/skills/`。当前版本为 1.3.0，支持可安装插件包和会话内动态插件；它不在默认推荐的 4 项中。升级旧版时，先验证新副本完整，再清理 `deepseek-harness-plugin-creator` 旧目录；`OtherSkills/cordis-plugin-development` 原文不单独安装。

---

## 四、各技能说明与触发方式

### 1. smilexx-coding-workflow（AI 编码工作流与代码规范）
- **作用**：覆盖需求分析、代码生成、审查、测试、交付，以及命名、注释、格式和错误处理等通用规范
- **触发**：编写、审查、重构、修复或规范化代码时自动使用
- **依赖**：无

### 2. smilexx-global-config（全局配置与 Agent 管理）
- **作用**：统一中文交互、文档规范、Agent 管理、任务并行协作和会话同步规则
- **触发**：安装后自动生效，无需手动调用
- **依赖**：无

### 3. smilexx-unity-development（Unity 开发）
- **作用**：Unity 项目开发指导（C#、场景设计、结构管理、构建部署），自动检测 Unity 版本与技术栈并查对应文档
- **触发**：说「Unity 开发」「Unity 项目」「帮我写 Unity C#」
- **依赖**：无（推荐本机安装 Unity 编辑器）

### 4. smilexx-unity-plugin-development（Unity 插件开发）
...
# 2. 重启 Codex Desktop
# 3. 测试触发，例如输入：「用 AI 帮我写一段代码」应激活 smilexx-coding-workflow
```
...
| `AI_Skill/skill-manifest.json` | 全部 9 个自有技能的安装清单 |
