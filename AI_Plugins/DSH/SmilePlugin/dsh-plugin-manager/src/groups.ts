 ...
 */
export async function loadGroups(dshHome): Promise<Record<string, any>> {
  try {
    const parsed: any = JSON.parse(await readFile(groupsFile(dshHome), "utf8"));
    if (parsed !== null && typeof parsed === "object" && parsed.groups !== null && typeof parsed.groups === "object") {
      const cleaned: Record<string, any> = {};
      for (const [id, group] of Object.entries(parsed.groups as Record<string, any>)) {
        if (group === null || typeof group !== "object" || typeof group.name !== "string") continue;
        const scopes: Record<string, any> = {};
        if (group.scopes !== null && typeof group.scopes === "object") {
          for (const [scope, names] of Object.entries(group.scopes as Record<string, any>)) {
            if (Array.isArray(names)) scopes[scope] = names.filter((name) => typeof name === "string");
          }
        }
 ...
 * @param name - 技能名。
 */
export function groupsForSkill(groups: any, scope: string, name: string): string[] {
  const result: string[] = [];
 ...
 * @returns 更新后的分组 id。
 */
export async function upsertGroup(dshHome, id, name: string, scope: string, names: string[]): Promise<string> {
  const trimmed = (name ?? "").trim();
  if (trimmed === "") throw new Error("分组名称不能为空");
  const groups = await loadGroups(dshHome);
  for (const [gid, group] of Object.entries(groups as Record<string, any>)) {
    if (gid !== id && group.name.toLowerCase() === trimmed.toLowerCase()) throw new Error('分组名 "' + trimmed + '" 已存在');
  }
 ...
 * @returns 是否真的删除了东西。
 */
export async function deleteGroup(dshHome, id: string): Promise<boolean> {
  const groups = await loadGroups(dshHome);
  if (groups[id] === undefined) return false;
  delete groups[id];
  await saveGroups(dshHome, groups);
  return true;
}
