  ...
  try {
    ...
    if (data !== null && typeof data === "object" && !Array.isArray(data)) {
      ...
      if (meta !== null && typeof meta === "object" && !Array.isArray(meta)) {
        if (typeof meta.category === "string" && meta.category !== "") category = meta.category;
        if (typeof meta.publisher === "string" && meta.publisher !== "") publisher = meta.publisher;
        if (Array.isArray(meta.platforms)) platforms = meta.platforms.filter((p) => typeof p === "string");
        if (Array.isArray(meta.keywords)) keywords = meta.keywords.filter((k) => typeof k === "string");
      }
    }
  } catch { /* 宽松解析失败不阻断 */ }
  return { name, description: pick("description") ?? "", whenToUse: pick("whenToUse"), body: body.trim(), modelInvocable: boolPick("disable-model-invocation") !== true, userInvocable: boolPick("user-invocable") !== false, category, publisher, platforms, keywords };
}
  ...
  for (const item of items) {
    ...
    if (isDir) {
      ...
      if (item.name.endsWith(".md" + DISABLED_SUFFIX)) {
        ...
        entries.push({ name: parsed?.name ?? item.name.slice(0, -(".md" + DISABLED_SUFFIX).length), description: parsed?.description ?? "", whenToUse: parsed?.whenToUse, enabled: false, kind: "flat", file: full, dirBundle: false, source: root.source, projectRoot: root.projectRoot, rel: "", category: parsed?.category, publisher: parsed?.publisher, platforms: parsed?.platforms, keywords: parsed?.keywords });
      } else if (item.name.endsWith(".md")) {
        const parsed = parseFrontmatter(await readFile(full, "utf8"));
        entries.push({ name: parsed?.name ?? item.name.slice(0, -3), description: parsed?.description ?? "", whenToUse: parsed?.whenToUse, enabled: true, kind: "flat", file: full, dirBundle: false, source: root.source, projectRoot: root.projectRoot, rel: "", category: parsed?.category, publisher: parsed?.publisher, platforms: parsed?.platforms, keywords: parsed?.keywords });
      }
    }
  }
}
