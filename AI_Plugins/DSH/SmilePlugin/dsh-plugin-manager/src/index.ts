...
class SkillsViewerGateway extends TypertRemoteService {
  ...
  async list(sessionId) {
    ...
    for (const entry of fileEntries) {
      ...
      skills.push({
        ...
        category: entry.category ?? (typeof entry.rel === "string" && entry.rel.includes("/") ? entry.rel.split("/")[0] : undefined),
        ...(entry.publisher ? { publisher: entry.publisher } : {}),
        ...(Array.isArray(entry.platforms) ? { platforms: entry.platforms } : {}),
  ...
  async setEnabled(name, sessionId, enabled, scope) {
    ...
    if (scoped) {
      const entry = await this.entryInScope(name, scope);
      if (entry !== undefined) return this.setEntryEnabled(name, entry, enabled);
  ...
  async migrate(name, sessionId, payload) {
    ...
    if (entry === undefined || entry === null) throw new Error('技能 "' + name + '" 没有可迁移的文件（随部署附带、运行时内置或不在指定源作用域的技能不可迁移）');
  ...
  async addSkill(sessionId, payload) {
    ...
    // zip 模式：解压并自动识别结构——唯一的顶层文件夹（内含 SKILL.md）按
    // 目录束处理，平铺的 .md 按单文件处理；非 zip 文件（如 .md）原样保留，
    // 与解压内容合并判定；其余结构一律拒绝。
    if (rawKind === "zip") {
      const unzipped: any[] = [];
      for (const file of decoded) {
        ...
        for (const name of Object.keys(zipEntries)) {
          ...
          if (name.startsWith("__MACOSX/") || name.split("/").includes("__MACOSX")) continue; // macOS 打包垃圾
          unzipped.push({ path: name.replaceAll("\\", "/"), data: Buffer.from(zipEntries[name]) });
        }
      }
      decoded = unzipped;
      if (decoded.length === 0) throw new Error("压缩包中没有可用的文件");
      if (decoded.length > MAX_ADD_FILES) throw new Error("压缩包内文件数量过多（最多 " + MAX_ADD_FILES + " 个）");
    ...
    if (kind === "bundle") {
      ...
      skills.push({
        ...
        writes: decoded.map((file) => ({ relative: file.path.slice(top.length + 1), data: file.path.toLowerCase().endsWith(".md") ? normalizeSkillText(file.data) : file.data }))
      });
    } else {
      ...
      for (const file of decoded) {
        ...
        skills.push({ name: validation.skill.name, writes: [{ relative: flatName, data: normalizeSkillText(file.data) }] });
      }
    }
    ...
    for (const skill of skills) {
      ...
      if (existing !== undefined) throw new Error('同名技能 "' + skill.name + '" 已存在（' + (existing.enabled ? "已启用" : "已停用") + "，位于 " + (existing.projectRoot !== undefined ? existing.projectRoot : "全局用户根") + "）");
    ...
    try {
      ...
      for (const target of written.reverse()) await rm(target, { recursive: true, force: true }).catch(() => {});
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
      ...
      try {
        for (const name of [...pending]) {
          if ((await registry.get(name, { cwd: probeCwd, scope })) !== undefined) pending.delete(name);
        }
