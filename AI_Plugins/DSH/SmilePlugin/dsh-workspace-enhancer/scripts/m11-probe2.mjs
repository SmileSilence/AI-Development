/**
 * m11-probe2：探测当前 GUI 的标签设置（标签列表 + 运行标签），
 * 供 m11 正式验证脚本设计确定性筛选场景。
 */
...
try {
  ...
  // 工作区组标签（第一层）
  const groups = await page.evaluate(() => Array.from(document.querySelectorAll('[role="treeitem"]'))
    .filter(el => el.hasAttribute('aria-expanded'))
    .map(el => (el.innerText || '').trim().slice(0, 40)))
  console.log('工作区组: ' + JSON.stringify(groups))
  ...
  await page.getByRole('button', { name: '添加条件' }).first().click({ timeout: 4000 })
  await page.waitForTimeout(400)
  const row = page.locator('.filterRuleRow').first()
  // 标签下拉（第 3 个锚点）
  await row.locator('button[aria-haspopup="listbox"]').nth(2).click({ timeout: 3000 })
  ...
  console.log('标签菜单: ' + JSON.stringify(menuText.replaceAll('\n', '|')))
  await browser.close()
} catch (err) {
  console.error('PROBE2 ERROR: ' + (err && err.message ? err.message : err))
