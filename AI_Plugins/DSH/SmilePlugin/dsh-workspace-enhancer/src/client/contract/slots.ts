...
import type { TaggerController } from '../tags/tagger-controller.ts'
...
export type WorkspaceBrowserInjected = {
  hooks: {
    ...
    /** 标签设置快照（集成自 dsh-workspace-tagger）：绑定为 useTagger 选择器 hook。 */
    tagger: HostObservable<SettingsScopeSnapshot<WorkspaceTaggerSettings>>
  }
  /**
   ...
   */
  agentPresetDefault: {
    /** 可选 preset（健康项，roster 顺序）；名册为空时返回空数组。 */
    list: () => Promise<{ id: string; name?: string; trust: 'system' | 'user' }[]>
    /** 当前默认 preset id；未设置时 undefined。 */
    current: () => Promise<string | undefined>
    /** 将某 preset 写为默认（写 settings `agent-presets.default`）。 */
    setDefault: (id: string) => Promise<void>
  }
  /** 标签子系统动作面（集成自 dsh-workspace-tagger）：CRUD / 分配 / 运行标签 / 比例 / 自定义色。 */
  tagger: TaggerController
  /** 标签命名空间文案（'workspace-tagger'）。 */
  taggerT: TranslateNS<'workspace-tagger'>
}
...
export type WorkspaceBrowserProps =
  ...
  & PropsLocale<'workspace-enhancer'>
